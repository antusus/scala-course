...
class FunSetSuite extends FunSuite {
    ...
    test("intersect contains elements") {
        new TestSets {
            ...
            assert(!contains(intersect(s1, s2), 1), "Intersect 2")
            assert(contains(intersect(union(s1, s2), s2), 2), "Intersect 3")
        }
    }

    test("diff contains elements") {
        new TestSets {
            assert(!contains(diff(s1, s1), 1), "Intersect 1")
            assert(contains(diff(s1, s2), 1), "Intersect 2")
            assert(!contains(diff(s1, s2), 2), "Intersect 3")
            assert(contains(diff(union(s1, s2), s2), 1), "Intersect 4")
